...
// Try to set DATABASE_URL if not set (for local SQLite)
if (!process.env.DATABASE_URL && !process.env.POSTGRES_PRISMA_URL) {
  ...
  if (require('fs').existsSync(dbPath)) {
    ...
    console.error('❌ DATABASE_URL not set and no local database found.');
    console.error('   Please set DATABASE_URL in .env.local or provide it as an environment variable.');
    process.exit(1);
  }
}
